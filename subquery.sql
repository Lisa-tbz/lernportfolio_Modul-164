...
SELECT * 
FROM buecher 
WHERE einkaufspreis = (SELECT MAX(einkaufspreis) FROM buecher);
SELECT * 
FROM buecher 
WHERE einkaufspreis = (SELECT MIN(einkaufspreis) FROM buecher);
SELECT * 
FROM buecher 
WHERE einkaufspreis > (SELECT AVG(einkaufspreis) FROM buecher);
SELECT b.*
FROM buecher b
JOIN buecher_has_sparten bh ON b.buecher_id = bh.buecher_buecher_id
JOIN sparten s ON bh.sparten_sparten_id = s.sparten_id
WHERE b.einkaufspreis > (
    SELECT AVG(b2.einkaufspreis)
    FROM buecher b2
    JOIN buecher_has_sparten bh2 ON b2.buecher_id = bh2.buecher_buecher_id
    JOIN sparten s2 ON bh2.sparten_sparten_id = s2.sparten_id
...
FROM (
    SELECT ahb.autoren_autoren_id, COUNT(*) AS anzahl
    FROM autoren_has_buecher ahb
    GROUP BY ahb.autoren_autoren_id
    HAVING COUNT(*) > 4
) AS bekannte;
SELECT a.vorname, a.nachname, COUNT(*) AS buchanzahl
FROM autoren a
JOIN autoren_has_buecher ahb ON a.autoren_id = ahb.autoren_autoren_id
GROUP BY a.autoren_id
HAVING COUNT(*) > 4;
SELECT AVG(gewinn_pro_buch) AS durchschnitt_gewinn
FROM (
    SELECT verlage_verlage_id, AVG(verkaufspreis - einkaufspreis) AS gewinn_pro_buch
    FROM buecher
    GROUP BY verlage_verlage_id
    HAVING AVG(verkaufspreis - einkaufspreis) < 10
) AS verlagsgewinne;
